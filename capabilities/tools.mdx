...
In [0mcp](https://0mcp.io), each tool is generated from an operation in your imported API source. The source can be an [OpenAPI specification](/api-sources/openapi), a [REST API definition](/api-sources/rest-api), or a [Postman collection](/api-sources/postman).

When you import your API, 0mcp creates tool previews for the available operations. You choose which tools to expose before creating and publishing the MCP server.
