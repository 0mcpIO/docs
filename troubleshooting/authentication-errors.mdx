...
For protected remote MCP servers, the official [MCP authorization specification](https://modelcontextprotocol.io/specification) defines the protocol-level authentication and token requirements.

0mcp uses pass-through authentication for upstream API credentials. For OAuth-protected MCP tools, the server **Setup** page may also require issuer configuration before client setup is usable.
...
- Use the gateway header provided by [0mcp](https://0mcp.io)
...
4. If Setup reports an OAuth configuration issue, add the issuer and JWKS settings from server **Settings**.
