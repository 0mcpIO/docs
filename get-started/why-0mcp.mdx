...
Building an MCP server from scratch means mapping API operations to tools, validating inputs, managing authentication, hosting infrastructure, and keeping everything in sync as your API evolves. [0mcp](https://0mcp.io) automates that workflow so you can publish an MCP server directly from your OpenAPI specification.
...
Select which API operations become [tools](/learn/core-concepts/tools), customize their names and descriptions, pause serving when needed, and publish versioned releases. Draft changes remain private until you choose to publish them.
